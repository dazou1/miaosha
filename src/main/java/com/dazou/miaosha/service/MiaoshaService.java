...
import com.dazou.miaosha.util.MD5Util;
import com.dazou.miaosha.util.UUIDUtil;
 ...
 * @Author: dazou
 * @Description:
 * @Date: Create in 12:43 2019-04-11
 */
...
public class MiaoshaService {
    ...
    //减少库存 下订单 秒杀订单 这三个操作必须是原子性，即放在一个事务里面，保证同时成功，否则失败
    @Transactional
    public OrderInfo miaosha(MiaoshaUser user, GoodsVo goods) {
        ...
        if (success) {
            ...
            setGoodsOver(goods.getId());//设置一个标志，利用此标志查看对应商品是否已经卖完
            return null;
        }
    ...
    public long getMiaoshaResult(Long userId, long goodsId) {
        ...
        if (order != null) {//秒杀成功
            ...
            boolean isOver = getGoodsOver(goodsId);//查看商品是否已经卖完了
            if (isOver) {
                return -1;//卖完了
            } else {
                return 0;//继续轮询，抢购
